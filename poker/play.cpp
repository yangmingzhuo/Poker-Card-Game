...
void play::gameOrder()//游戏进程
{
    ...
    if(order%3 == 0)
    {
        ...
        ui->deal->setEnabled(1);
        ui->label_remind->setText("Your turn");
        deal = 0;
    }
...
void play::on_deal_clicked()//出牌，检测是否符合规则
{
    ...
    {
        ui->label_remind->setText("Your combo does not fit the rule!!!");
        invisibleAllCards();
        showCardsInHand(A);
    }
}
...
void play::on_prompt_clicked() //提示 已知卡牌数量 标志卡牌 用排列组合
{
    if(kind != 20)
    {
        ...
        else ui->label_remind->setText("No combo fit");
    }
    else ui->label_remind->setText("Choose your combo!!! ");
}
